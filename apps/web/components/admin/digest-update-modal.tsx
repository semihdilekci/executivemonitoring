...
export function DigestUpdateModal({
  ...
  const { data: templates, isLoading: templatesLoading } =
    useNewsletterTemplates({ is_active: true });
  const [newsletterTemplateId, setNewsletterTemplateId] = useState("");
  ...
  // Şablonlar yüklenince ilk bülteni varsayılan seç.
  useEffect(() => {
    if (!isOpen || !templates || templates.length === 0) return;
    setNewsletterTemplateId((current) =>
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!newsletterTemplateId) {
      setFieldError("Bir bülten seçin.");
      return;
    }
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Bülten Güncelle">
      <form onSubmit={(event) => void handleSubmit(event)} className="space-y-4" noValidate>
        ...
        <div className="space-y-1.5">
          <label htmlFor="newsletter-template" className="block text-sm font-medium text-gray-700">
            Bülten
          </label>
          <select
            id="newsletter-template"
            value={newsletterTemplateId}
            disabled={isSubmitting || templatesLoading || !templates?.length}
            onChange={(event) => setNewsletterTemplateId(event.target.value)}
            className="flex h-10 w-full rounded-md border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-navy-600 disabled:opacity-60"
          >
            {templatesLoading ? (
              <option value="">Yükleniyor…</option>
            ) : !templates?.length ? (
              <option value="">Aktif bülten bulunamadı</option>
            ) : (
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          <Button
            type="submit"
            disabled={isSubmitting || !newsletterTemplateId}
