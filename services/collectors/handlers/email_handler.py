"""Email collector Lambda handler."""
...
EMAIL_SOURCE_TYPE = SourceType.EMAIL.value


def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    """EventBridge → email collector Lambda giriş noktası."""
    import asyncio
    ...
    async def _run() -> dict[str, int]:
        sources = await sources_loader(EMAIL_SOURCE_TYPE)
        return await run_collector_batch(EMAIL_SOURCE_TYPE, sources)
